...
struct RobotConfig {
 ...
 // TODO: decide if these should be hardcoded in the struct definition.
  ...
  Expr<wpi::units::kilogram_square_meter_t> inertia =6.0_kg_sq_m;  // kg-m^2, estimated moment of inertia of a typical FRC robot
  Expr<wpi::units::scalar_t> gearing = 6.5;
  Expr<wpi::units::meter_t> radius = 2_in;  // 2 inches
  Expr<wpi::units::radians_per_second_t> vmax = 6000.0_rpm;  // 5400 RPM free speed of a typical FRC motor
  Expr<wpi::units::newton_meter_t> tmax = 1.2_Nm;  // N-m, estimated stall torque of a typical FRC motor with gearing
  Expr<wpi::units::scalar_t> cof = 1.5;
  Expr<wpi::units::meter_t> differential_track_width = 22_in;  // m, distance between left and right wheels on a typical FRC robot
  /// FL, BL, BR, FR
  std::vector<WheelLocation> wheels = {{+11_in, +11_in}, {+11_in, -11_in}, {-11_in, -11_in}, {-11_in, +11_in}};
  ...
  static RobotConfig default_frc_swerve() {
    return RobotConfig{
     ...
     .wheels = {{+11_in, +11_in}, {+11_in, -11_in}, {-11_in, -11_in}, {-11_in, +11_in}}
    };
  }
  ...
  trajopt::SwerveDrivetrain to_swerve_drivetrain() {
    ...
    std::transform(wheels.begin(), wheels.end(),
                   std::back_inserter(trajoptModules),
                   [](WheelLocation module){return trajopt::Translation2d(module);});
    return trajopt::SwerveDrivetrain{.mass = mass,
                                     .moi = inertia,
                                     // m
                                     .wheel_radius = radius,
                                     // rad/s
  ...
  trajopt::DifferentialDrivetrain to_differential_drivetrain() {
    return trajopt::DifferentialDrivetrain{.mass = mass,
                                         .moi = inertia,
                                         // m
                                         .wheel_radius = radius,
                                         // rad/s
...
void to_json(wpi::util::json& json, const RobotConfig& config) {
  json = wpi::util::json::object(
    ...
    "tmax", config.tmax,
    "cof", config.cof,
    "differential_track_width", config.differential_track_width,
...
void from_json(const wpi::util::json& json, RobotConfig& config) {
  ...
  config.inertia = json.at("inertia").get<choreo::Expr<wpi::units::kilogram_square_meter_t>>();
  ...
  config.vmax = json.at("vmax").get<choreo::Expr<wpi::units::radians_per_second_t>>();
  ...
  config.differential_track_width = json.at("differential_track_width").get<choreo::Expr<wpi::units::meter_t>>();
  config.wheels.clear();
  auto whs = json.at("wheels").get_array();
  std::transform(whs.begin(), whs.end(), std::back_inserter(config.wheels), [](auto modJson){return modJson.get<WheelLocation>();});
