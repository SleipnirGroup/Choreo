<script lang="ts">
  import type { Path } from "$lib/path.svelte.js";
  import {
    type Waypoint,
    getWaypoint
  } from "$lib/waypoint.svelte.js";
  import styles from "./Sidebar.module.css";
  import SidebarWaypoint from "./SidebarWaypoint.svelte";
  let { path }: { path: Path } = $props();
  let waypoints = $derived(path.waypoints);
  $inspect(waypoints).with(console.trace);
  $inspect(waypoints[0]?.x()).with(console.trace);
...
<div>
  {#each waypoints as point, i (point.id)}
    {#if point !== undefined}
      <SidebarWaypoint
        waypoint={point}
        index={i}
        ...
        handleDelete={(id)=>path.deleteWaypoint(id)}
      ></SidebarWaypoint>
    {/if}
  {/each}
  {#if waypoints.length == 0}
    <div class={"SidebarItem Noninteractible"}>
      <span></span>
      <span style="color: gray; font-style: italic"> No Waypoints </span>
    </div>
  {/if}
</div>
