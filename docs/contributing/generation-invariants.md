...
* `trajectory` is fully dependent on the generator result, the `snapshot` waypoints' `.intervals` and `.split` fields, the project type from the project file, and the robot config from the project file.
  ...
  * `trajectory.config` is a snapshot of the robot config.
