...
pub fn adjust_headings(params: &mut Parameters<f64>) -> ChoreoResult<()> {
    let new_headings = calculate_adjusted_headings(params)?;
    // new_headings, set to file
    for (i, &h) in new_headings.iter().enumerate() {
        params.waypoints[i].heading = h;
    }
    Ok(())
}

pub fn calculate_adjusted_headings(params: &Parameters<f64>) -> ChoreoResult<Vec<f64>> {
    ...
    let (_, constraints_idx) = fix_constraint_indices(params);

    // find pose waypoints
    for (w, maybe_h) in waypoints.iter().zip(headings_from_constraints.iter_mut()) {
        if w.fix_heading {
            *maybe_h = Some(w.heading);
        }
    }
    ...
    let mut last_fixed_heading = (0, waypoints[0].heading);
    ...
    if let Some((i, _)) = waypoints
        ...
        .find(|(i, w)| w.heading != new_headings[*i])
    {
        return Err(ChoreoError::HeadingConflict(
            i + 1,
...
// This is duplicated in constraints::ConstraintSetter
pub fn fix_constraint_indices(params: &Parameters<f64>) -> (Vec<usize>, Vec<ConstraintIDX<f64>>) {
