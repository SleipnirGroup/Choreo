...
impl NameErr {
        ...
        NameErr::InvalidChar { name: _, character } => {
          let invalid_chars: String = 
            character.iter()
              ...
              .join(" ");
          format!(
            "Can only use letters, 0-9, and _. Can't use {invalid_chars}"
          )
        },
  ...
  pub fn codegen_msg(&self) -> String {
    match &self {
        NameErr::Empty | NameErr::Exists(_) => 
          String::from("This error should never appear in generated code. Tell the developers."),
...
pub fn validate_name(name: &str) -> Result<(), NameErr> {
  static FIRST_CHAR_RE: LazyLock<Regex> = LazyLock::new(|| {
      Regex::new(r"[0-9_]").expect("Invalid regex")
  });
  static ALL_CHARS_RE: LazyLock<Regex> = LazyLock::new(|| {
  ...
  let invalid_chars: Vec<String> = 
    ALL_CHARS_RE.find_iter(name)
      .map(|mat| mat.as_str())
      .filter(|item| {
