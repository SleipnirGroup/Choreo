...
function FieldConstraintRangeLayer(props: Props) {
    ...
    if (props.start !== undefined) {
        ...
        const points = props.points.slice(props.start, (props.end ?? props.start) + 1);
        return <g>
            <g fill="none" stroke={lineColor}>
            <g id={`range-circles-${id}`} visibility={props.showCircles?"show":"none"}>
                {points.map((w, i) => {
                    return <circle
                            cx={w.x.value}
                            cy={w.y.value}
                            ...
                            style={
                                (props.onCircleClick === undefined &&
                                    ...
                                    props.onCircleMouseOver === undefined)
                                     ? { "pointerEvents": "none" } : { "pointerEvents": "visible" }}
                        ></circle>
            ...
            <mask id={`range-clip-${id}`}>
                ...
                <use href={`#range-circles-${id}`} fill="black" stroke="black"></use>
            </mask>
            </defs>
