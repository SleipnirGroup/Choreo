...
public final class ChoreoConsts {`);
    ...
    const maxLinearVel = config.vmax.val / config.gearing.val * config.radius.val;
    const maxWheelForce = config.tmax.val * config.gearing.val * 4 / config.radius.val;
    ...
    public static final Translation2d[] moduleTranslations = {
        ...
        const drivebaseRadius = Math.hypot(config.frontLeft.x.val, config.frontLeft.y.val);
        ...
        const maxAngularAccel = minLinearForce * drivebaseRadius / config.inertia.val;
