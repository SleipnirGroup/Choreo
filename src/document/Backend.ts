...
export const Commands = {
  guessIntervals: (config: RobotConfig<Expr>, traj: TrajFile) =>
   ...
   */
  generate: (project: ProjectFile, traj: TrajFile, handle: number) =>
    invoke<TrajFile>("generate_remote", { project, traj, handle }),
   ...
   */
  defaultProject: () => invoke<ProjectFile>("default_project"),
  /**
   ...
   */
  readProject: (name: string) => invoke<ProjectFile>("read_project", { name }),
  /**
   ...
   */
  readTraj: (name: string) => invoke<TrajFile>("read_traj", { name }),
  /**
   ...
   */
  readAllTraj: () => invoke<TrajFile[]>("read_all_traj"),
  /**
   ...
   * @returns `void`
   */
  writeTraj: (traj: TrajFile) => invoke("write_traj", { traj }),
  /**
   ...
   * @returns `void`
   */
  renameTraj: (oldTraj: TrajFile, newName: string) =>
   ...
   * @returns `void`
   */
  deleteTraj: (traj: TrajFile) => invoke<void>("delete_traj", { traj }),
   ...
   */
  openDiagnosticZip: (project: ProjectFile, trajs: TrajFile[]) =>
