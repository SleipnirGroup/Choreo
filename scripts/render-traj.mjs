...
// Match FieldImage2026.svg's viewBox so embedded background lines up.
...
const fieldB64 = readFileSync(fieldPath).toString("base64");

const traj = JSON.parse(readFileSync(argv.traj, "utf8")).trajectory;
const samples = traj.samples ?? [];

if (samples.length < 2) {
  writeFileSync(argv.out, emptySvg(argv.title, fieldB64));
  process.exit(0);
}
...
const out = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="${VB_X} ${VB_Y} ${VB_W} ${VB_H}">
  <image x="${VB_X}" y="${VB_Y}" width="${VB_W}" height="${VB_H}" href="data:image/svg+xml;base64,${fieldB64}" preserveAspectRatio="none"/>
...
function emptySvg(title, fieldB64) {
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="${VB_X} ${VB_Y} ${VB_W} ${VB_H}">
  <image x="${VB_X}" y="${VB_Y}" width="${VB_W}" height="${VB_H}" href="data:image/svg+xml;base64,${fieldB64}" preserveAspectRatio="none"/>
