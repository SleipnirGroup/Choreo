...
inline std::vector<Segment> estimate_segment_times(const std::vector<Segment>& segments, const RobotConfig& config) {
    std::vector<Segment> estimated_segments;
    for (int i = 0; i < segments.size()-1; i++) {
        estimated_segments.push_back(estimate_segment_time(segments[i], segments[i+1], config));
    };
    estimated_segments.push_back(segments.back());
    estimated_segments.back().estimated_time = 0_s;  // this must be at least one interval though.
    return estimated_segments;
}

template<typename Drivetrain, typename Solution>
void apply_segments(const std::vector<choreo::Segment>& segments, trajopt::PathBuilder<Drivetrain, Solution>& generator, const wpi::units::second_t target_dt) {
  size_t wpt_cnt = 0;
  ...
  for (int i = 0; i < segments.size(); i++) {
    ...
    std::println("Applying segment starting at waypoint ({}, {}) with heading {} and {} segment constraints and {} waypoint constraints",
      segment.start.x.val.value(), segment.start.y.val.value(), segment.start.heading.val.value(), segment.segment_constraints.size(), segment.waypoint_constraints.size());
      if (segment.coalesce_with_previous) {
        std::println("This segment is marked to be coalesced with the previous segment.");
