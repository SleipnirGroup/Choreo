...
void choreo::from_json(const wpi::json& json,
  ...
  trajectory.samples = json.at("trajectory").at("samples").get<std::vector<SwerveSample>>();
  trajectory.splits = json.at("trajectory").at("splits").get<std::vector<int>>();
...
void choreo::from_json(const wpi::json& json,
  ...
  trajectory.samples = json.at("trajectory").at("samples").get<std::vector<DifferentialSample>>();
  trajectory.splits = json.at("trajectory").at("splits").get<std::vector<int>>();
  trajectory.events = json.at("events").get<std::vector<EventMarker>>();
}
