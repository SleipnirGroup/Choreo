...
public class ChoreoAutoTrajectory {
  ...
  private static final double DEFAULT_TOLERANCE_METERS = 3.0 * 0.0254;
  ...
  public Trigger atTime(double timeSinceStart) {
    ...
    if (timeSinceStart > trajectory.getTotalTime()) {
      DriverStation.reportWarning("Trigger time cannot be greater than total trajectory time for " + trajName, true);
      return new Trigger(loop, () -> false);
    }
   ...
   * @return A trigger that is true when the event with the given name has been reached based on time.
   * @apiNote A warning will be printed to the DriverStation if the event is not found.
   */
   ...
   * @return A trigger that is true when the robot is within toleranceMeters of the given events pose.
   */
  public Trigger atPose(String eventName, double toleranceMeters) {
    var optEvent = trajectory.getEvent(eventName);
    if (optEvent.isPresent()) {
      Pose2d pose = trajectory.sample(optEvent.get().timestamp, mirrorTrajectory.getAsBoolean()).getPose();
   ...
   * @param eventName The name of the event.
   * @param toleranceMeters The tolerance in meters.
   * @return A trigger that is true when the event with the given name has been reached based on time and the robot is within toleranceMeters of the given events pose.
   * @apiNote A warning will be printed to the DriverStation if the event is not found.
   */
   ...
   * 
   * @param eventName The name of the event.
   * @return A trigger that is true when the event with the given name has been reached based on time and the robot is within 3 inches of the given events pose.
   * @apiNote A warning will be printed to the DriverStation if the event is not found.
   */
