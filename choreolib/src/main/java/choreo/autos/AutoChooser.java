 ...
 * 
 * <p>This chooser takes a lazy loading approach to auto routines,
 * only generating the auto routine when it is selected.
 * This approach has the benefit of not loading all autos on startup,
 * but also not loading the auto during auto start causing a delay.
 * 
 * <p>Once the {@link AutoChooser} is made you can add auto routines to it
 * using the {@link #addAutoRoutine(String, AutoRoutineGenerator)} method.
 * Unlike {@code SendableChooser} this chooser has to be updated every cycle
 * by calling the {@link #update()} method in your {@link IterativeRobotBase#robotPeriodic()}.
 * 
 * <p> You can retrieve the auto routine {@link Command} that is currently selected
 * by calling the {@link #getSelectedAutoRoutine()} method.
 */
public class AutoChooser {
     ...
     *                 passing in an empty string will put this chooser at the root of the network tables.
     */
    public AutoChooser(AutoFactory factory, String tableName) {
        ...
        options = table.getStringArrayTopic("options")
            .getEntry(autoRoutines.keySet().toArray(new String[0]));
    }
     ...
     * 
     * <p> An auto routine is a function that takes an AutoFactory and returns a Command.
     ...
     * subsystems and has helper methods for auto commands inside it. Then you crate methods
     * inside that class that take an `AutoFactory` and return a `Command`.
     * 
