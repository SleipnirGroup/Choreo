...
import choreo.util.AlertUtil.MultiAlert;
...
public class AutoTrajectory {
  ...
  private static final MultiAlert triggerTimeNegative =
      AlertUtil.multiAlert(causes -> "Trigger time cannot be negative for " + causes, kError);
  private static final MultiAlert triggerTimeAboveMax =
      AlertUtil.multiAlert(
          causes -> "Trigger time cannot be greater than total trajectory time for " + causes + ".",
          kError);
  private static final MultiAlert eventNotFound =
      AlertUtil.multiAlert(causes -> "Event Markers " + causes + " not found.", kError);
  private static final MultiAlert noSamples =
      AlertUtil.multiAlert(causes -> "Trajectories " + causes + " have no samples.", kError);
