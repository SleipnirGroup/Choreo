...
namespace choreo {
    namespace render {
        ...
        namespace path_gradient {
            struct PathGradient {
                std::function<HSL(std::vector<choreo::SwerveSample>, size_t index)> swerve_color;
                HSL operator()(std::vector<choreo::SwerveSample>samples, size_t index) {
                    return swerve_color(samples, index);
                }
            };
            static PathGradient progress{
                [](auto samples, auto index){
                auto interp = ((double)index/(double)samples.size());

                return HSL{120.0/360.0 * (1-interp), 1, 0.5};
            }};
            static PathGradient linearVelocity{
                [](auto samples, auto index){
