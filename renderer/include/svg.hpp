...
#define SVG_TYPE_CHECK                              \
  static_assert(std::is_base_of<Element, T>::value, \
                "Child must be an SVG element.")
#define APPROX_EQUALS(x, y, tol) static_cast<bool>(abs((x) - (y)) < (tol))
#include <math.h>  // NAN
...
inline std::string tag_name(ElementKind kind) {
  switch (kind) {
    case ElementKind::Defs:
      return "defs";
    case ElementKind::Symbol:
      return "symbol";
    case ElementKind::Use:
      return "use";
    case ElementKind::SVG:
      return "svg";
    case ElementKind::Style:
      return "style";
...
static const Margins DEFAULT_MARGINS = {10, 10, 10, 10};
static const Margins NO_MARGINS = {0, 0, 0, 0};
...
inline std::string to_string(const std::map<std::string, AttributeMap>& css,
...
SVG merge(std::vector<SVG>& frames, const double width,
...
class ClassList {
 public:
  ClassList() = default;
  explicit ClassList(std::string& value)
      : mutable_value_(&value), value_(&value) {}
  ...
  ClassList& remove(const std::string& token) {
    ...
    values.erase(std::remove(values.begin(), values.end(), token),
                 values.end());
  ...
  /** Add a missing token or remove an existing token, returning true when
   * present */
  ...
  std::string str() const { return join(tokens()); }

  /** Return the current tokens in order */
  std::vector<std::string> tokens() const { return parse(value()); }
...
class TransformList {
 public:
  ...
  explicit TransformList(std::string& value)
      : mutable_value_(&value), value_(&value) {}
  ...
  TransformList& matrix(double a, double b, double c, double d, double e,
                        double f) {
    std::stringstream ss;
    ss << "matrix(" << to_string(a) << " " << to_string(b) << " "
 ...
 private:
  static bool is_keyword(const std::string& transform) {
    return transform == "inherit" || transform == "initial" ||
           transform == "revert" || transform == "revert-layer" ||
           transform == "unset";
  }
...
/** @class AttributeMap
 *  @brief Base class for anything that has attributes (e.g. SVG elements, CSS
 * stylesheets)
 */
class AttributeMap {
 public:
  struct AttrSetter {
    AttrSetter(SVGAttrib::mapped_type& _attr, bool _normalize_class = false,
        ...
        : attr_(_attr),
          normalize_class_(_normalize_class),
          on_update_(_on_update) {};
  ...
  explicit AttributeMap(SVGAttrib _attr) : attr_(std::move(_attr)) {};

  const SVGAttrib& attrs() const { return this->attr_; }
  ...
  ClassList class_list() { return ClassList(this->attr_["class"]); }
  ...
  TransformList transform_list() const {
    ...
    return found == this->attr_.end() ? TransformList()
                                      : TransformList(found->second);
  }

  TransformList transform() { return transform_list(); }

  TransformList transform() const { return transform_list(); }

 protected:
  virtual void set_attr_value(const std::string& key,
  ...
  virtual AttrSetter make_attr_setter(const std::string& key) {
    if (this->attr_.find(key) == this->attr_.end())
      this->attr_[key] = "";
    return AttrSetter(this->attr_.at(key), key == "class");
  }

  SVGAttrib& mutable_attrs() { return this->attr_; }
...
template <>
inline AttributeMap::AttrSetter& AttributeMap::AttrSetter::operator<<(
    const char* value) {
  attr_ += value;
  normalize();
  notify();
...
template <>
inline AttributeMap& AttributeMap::set_attr(const std::string key,
                                            const double value) {
...
class Element : public AttributeMap {
 public:
  /** @class BoundingBox
   *  @brief Represents the top left and bottom right corners of a bounding
   * rectangle
   */
  ...
  explicit Element(const char* id) : AttributeMap(SVGAttrib({{"id", id}})) {};
  ...
  std::vector<T*> get_children() {
    ...
    for (auto& child : child_elems)
      if (child->kind() == T::static_kind)
        ret.push_back(static_cast<T*>(child));
  ...
  std::vector<T*> get_immediate_children() {
    ...
    for (auto& child : this->children) {
      if (child->kind() == T::static_kind)
        ret.push_back(static_cast<T*>(child.get()));
    }
 ...
 protected:
  std::vector<std::unique_ptr<Element>>
      children; /** Smart pointers to child elements */
  ...
  virtual std::string svg_to_string(
      const size_t
          indent_level); /** SVG string corresponding to this element */
  virtual std::string tag() {
    return tag_name(this->kind());
  } /** The SVG tag of this element */

  void set_attr_value(const std::string& key,
  ...
  Element* insert_child(std::unique_ptr<Element> child,
                        ChildIterator position) {
    child->parent_ = this;
...
inline std::vector<Element*> Element::get_elements_by_class(
  ...
  for (auto& current : child_elems) {
    if (current->has_attr("class") && current->class_list().contains(clsname))
...
class Shape : public Element {
 public:
  ...
  virtual std::vector<Point> points() {
    /** Return a set of points used for calculating a bounding polygon for this
     * object */
...
class Symbol : public Element {
 public:
  ...
  explicit Symbol(std::string id) { this->id(id); }
...
class Use : public Shape {
 public:
  ...
  explicit Use(std::string href) { set_attr("href", std::move(href)); }
 ...
 public:
  class Style : public Element {
   public:
    ...
    std::map<std::string, SelectorProperties>
  ...
  explicit SVG(SVGAttrib _attr = {{"xmlns", "http://www.w3.org/2000/svg"}})
      : Shape(_attr) {
    set_owner_svg(this);
    rebuild_id_index();
  };
  ...
  Defs* defs() {
    if (!this->defs_)
      this->defs_ = this->add_child<Defs>();
    return this->defs_;
  }

  template <typename T, typename... Args>
  typename std::enable_if<!std::is_same<T, Defs>::value, T*>::type add_child(
      Args&&... args) {
  ...
  T* get_element_by_id(const std::string& id) {
    ...
    if (!element || element->kind() != T::static_kind)
      return nullptr;
    return static_cast<T*>(element);
  }

  Style* css =
      this->add_child<Style>(); /**< This item's associated CSS stylesheet */
  ElementKind kind() const override { return static_kind; }
...
inline void Element::set_attr_value(const std::string& key,
                                    const std::string& value) {
  if (key == "id") {
    this->id(value);
    return;
...
inline AttributeMap::AttrSetter Element::make_attr_setter(
  ...
  return AttrSetter(this->mutable_attrs().at(key), false,
...
class Line : public Shape {
 public:
  ...
  double x() override { return x1() + (x2() - x1()) / 2; }
  double y() override { return y1() + (y2() - y1()) / 2; }
...
class Circle : public Shape {
 public:
  ...
  double x() override { return this->find_numeric("cx"); }
  double y() override { return this->find_numeric("cy"); }
  double width() override { return this->radius() * 2; }
  double height() override { return this->width(); }
  Element::BoundingBox get_bbox() override;
};
...
inline Element::BoundingBox Rect::get_bbox() {
  double x = this->x(), y = this->y(), width = this->width(),
         height = this->height();
...
inline std::pair<double, double> Line::along(double percent) {
  ...
  if (x1() != x2()) {
    ...
    double discrim =
        std::sqrt(4 * pow(length, 2) * (1 / (1 + pow(slope(), 2))));
...
inline std::string SVG::Style::svg_to_string(const size_t indent_level) {
  ...
  if (!this->css.empty() || !this->media_queries.empty() ||
      !this->keyframes.empty()) {
    std::string ret =
    ...
    for (auto& media : this->media_queries) {
      ret += indent + "\t\t@media " + media.first + " {\n" +
             to_string(media.second, indent_level + 1) + indent + "\t\t" +
             "}\n";
    }
...
inline void Element::autoscale(const double margin) {
  ...
  this->autoscale(
      {width * margin, width * margin, height * margin, height * margin});
}
...
inline void Element::autoscale(const Margins& margins) {
  ...
  this->set_attr("width", width).set_attr("height", height);

  if (x1 < 0 || y1 < 0) {
    std::stringstream viewbox;
    viewbox << std::fixed << std::setprecision(1) << x1 << " "  // min-x
            << y1 << " "                                        // min-y
            << width << " " << height;
...
inline std::vector<Element*> Element::get_children_helper() {
  /** Helper function which populates a std::deque with all of an Element's
   * children */
...
inline std::vector<Point> bounding_polygon(std::vector<Shape*>& shapes) {
  ...
  for (auto& shp : shapes) {
    ...
    std::move(temp_points.begin(), temp_points.end(),
...
inline SVG merge(std::vector<SVG>& frames, const double width,
                 const int max_frame_width) {
  ...
  for (auto& frame : frames) {
    ...
    if (frame.width() > max_frame_width) {
      ...
      frame.set_attr(
          "height",
          frame.height() * scale_factor);  // Scale height proportionally
    }
  }
  ...
  // Set viewbox
  root.set_attr("viewBox") << 0 << " " << 0 << " " << total_width << " "
                           << total_height;
...
inline SVG frame_animate(std::vector<SVG>& frames, const double fps) {
  ...
  const double duration =
      static_cast<double>(frames.size()) / fps;  // [seconds]
  const double frame_step = 1.0 / fps;  // duration of each frame [seconds]
  int current_frame = 0;

  root.style("svg.animated")
      .set_attr("animation-iteration-count", "infinite")
  ...
  for (auto& frame : frames) {
    ...
    root.style("#" + frame_id)
        .set_attr("animation-name", "anim_" + std::to_string(current_frame));
    current_frame++;
    root << std::move(frame);
  }
  ...
  for (size_t i = 0, ilen = frames.size(); i < ilen; i++) {
    ...
    double begin_pct = static_cast<double>(i) / frames.size(),
           end_pct = static_cast<double>(i + 1) / frames.size();
  ...
  root.set_attr("viewBox",
                "0 0 " + std::to_string(width) + " " + std::to_string(height));

  // Center child SVGs
  for (auto& child : root.get_immediate_children<SVG>())
    child->set_attr("x", (width - child->width()) / 2)
        .set_attr("y", (height - child->height()) / 2);
