...
template <size_t I, typename T, size_t N>
  requires(I < N)
constexpr T& get(trajopt::array<T, N>& arr) noexcept {
...
template <size_t I, typename T, size_t N>
  requires(I < N)
constexpr const T& get(const trajopt::array<T, N>& arr) noexcept {
  return std::get<I>(static_cast<std::array<T, N>>(arr));
}
...
// Partial specialization for trajopt::array
template <typename T, size_t N>
struct tuple_size<trajopt::array<T, N>> : public integral_constant<size_t, N> {};

// Partial specialization for trajopt::array
template <size_t I, typename T, size_t N>
  requires(I < N)
struct tuple_element<I, trajopt::array<T, N>> {
  using type = T;
};
}  // namespace std
