...
template <typename T>
concept SampleLike =
    requires(T sample, wpi::math::Transform2d transform,
             wpi::math::Pose2d otherPose, wpi::units::second_t newTimestamp) {
      ...
      {
        sample.acceleration
      } -> std::convertible_to<wpi::math::ChassisAccelerations>;
...
concept TrajectoryLike =
    requires(T traj, std::vector<Sample> samples, const Sample& start,
             const Sample& end, const wpi::math::Transform2d& transform,
             const wpi::math::Pose2d& pose, const T& otherTraj) {
      T(samples);
...
concept DriveTypeLike = requires(T::TrajoptSample trajoptSample) {
  ...
  {
    T::fromTrajopt(trajoptSample)
  } -> std::convertible_to<typename T::WPILibSample>;
};
#else
