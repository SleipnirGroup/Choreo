...
#include "sample_variant.hpp"
  ...
  inline void to_json(wpi::util::json & json, const Trajectory& traj) {
    const wpi::util::json config_json = traj.config ?
        wpi::util::json(*traj.config) : wpi::util::json();
    const wpi::util::json sample_type_json = traj.sample_type ?
        wpi::util::json(*traj.sample_type) : wpi::util::json();
    json = wpi::util::json::object(
        "config", config_json, "sample_type", sample_type_json,
        "waypoints", traj.waypoints, "samples", traj.samples, "splits",
        traj.splits);
  }
