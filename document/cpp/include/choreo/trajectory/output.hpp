...
struct Trajectory {
  ...
  static inline Trajectory<Type> realDefault() {
    return Trajectory<Type>{{},
                            typename Type::WPILibTrajectory{
                                std::vector<typename Type::WPILibSample>{}},
                            {}};
  }
  static inline Trajectory<Type> from_json(const wpi::util::json& json) {
    std::string s = json.at("sample_type").get_string();
    if (s != Type::tag) {
      throw std::invalid_argument("Parsing Trajectory with wrong drive type" +
                                  s);
    }

    auto sampleVec = json.at("samples")
                         .at("samples")
                         .get<std::vector<typename Type::WPILibSample>>();
...
inline void to_json(wpi::util::json& json, const Trajectory<Type>& traj) {
  auto waypoints =
      traj.waypoints |
      std::views::transform([](wpi::units::second_t t) { return t.value(); }) |
      std::ranges::to<std::vector<double>>();
  json = wpi::util::json::object("waypoints", waypoints, "splits", traj.splits);
