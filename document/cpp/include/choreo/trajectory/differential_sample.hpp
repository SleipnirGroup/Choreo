...
inline void to_json(wpi::util::json& json, const DifferentialSample& sample) {
  json = wpi::util::json::object(
      ...
      "heading", sample.heading.value(), "vl", sample.vl.value(),
      "vr", sample.vr.value(), "omega", sample.omega.value(), "al",
      sample.al.value(), "ar", sample.ar.value(), "alpha",
      sample.alpha.value(), "fl", sample.fl.value(), "fr", sample.fr.value());
}
inline void from_json(const wpi::util::json& json, DifferentialSample& sample) {
  ...
  sample.heading = static_cast<wpi::units::radian_t>(json.at("heading").get_number());
  sample.vl = static_cast<wpi::units::meters_per_second_t>(json.at("vl").get_number());
  sample.vr = static_cast<wpi::units::meters_per_second_t>(json.at("vr").get_number());
  sample.omega = static_cast<wpi::units::radian_t>(json.at("omega").get_number());
  sample.al = static_cast<wpi::units::meters_per_second_squared_t>(json.at("al").get_number());
  sample.ar = static_cast<wpi::units::meters_per_second_squared_t>(json.at("ar").get_number());
  sample.alpha = static_cast<wpi::units::radian_t>(json.at("alpha").get_number());
