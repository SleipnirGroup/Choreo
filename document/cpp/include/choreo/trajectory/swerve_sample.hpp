...
struct SwerveDriveType {
  ...
  static_assert(SampleLike<WPILibSample>,
                "SwerveDriveType::WPILibSample must match SampleLike");
  ...
  constexpr static WPILibSample fromTrajopt(const TrajoptSample& sample) {
    return WPILibSample(
        ...
        wpi::math::Pose2d{
            wpi::units::meter_t(sample.x), wpi::units::meter_t(sample.y),
            wpi::math::Rotation2d(wpi::units::radian_t(sample.heading))},
        {wpi::units::meters_per_second_t(sample.velocity_x),
         wpi::units::meters_per_second_t(sample.velocity_y),
         wpi::units::radians_per_second_t(sample.angular_velocity)},
        {wpi::units::meters_per_second_squared_t(sample.acceleration_x),
         wpi::units::meters_per_second_squared_t(sample.acceleration_y),
         wpi::units::radians_per_second_squared_t(
             sample.angular_acceleration)});
  }
#endif
};
static_assert(DriveTypeLike<SwerveDriveType>,
              "SwerveDriveType must satisfy DriveTypeLike");
}  // namespace choreo
